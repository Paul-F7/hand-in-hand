...
export function LessonPath() {
  ...
  const [streak, setStreak] = useState<number>(() => loadFromStorage("asl_streak", 1));
  ...
  const buildAndShowResults = useCallback(() => {
    ...
    const passThreshold = Math.ceil(totalQuestions * 0.66); // 66% to pass
    const passed = totalCorrect >= passThreshold;

    console.log('📊 Lesson Results:', { totalCorrect, totalQuestions, passThreshold, passed });
  ...
  const handleResultsContinue = useCallback(() => {
    ...
    if (resultsData.passed) {
      if (!review) {
        // Update completed lessons and save immediately to localStorage
        setCompletedLessons(prev => {
          const s = new Set(prev);
          s.add(lessonIndex);
          // Save immediately to ensure persistence
          localStorage.setItem("asl_completedLessons", JSON.stringify(Array.from(s)));
          console.log('✅ Lesson completed! Saved to localStorage:', Array.from(s));
          return s;
        });

        if (resultsData.xpEarned > 0) {
          setTotalXP(prev => {
            const newXP = prev + resultsData.xpEarned;
            localStorage.setItem("asl_totalXP", JSON.stringify(newXP));
            return newXP;
          });
          setDailyGoal(prev => {
            const newGoal = prev + resultsData.xpEarned;
            localStorage.setItem("asl_dailyGoal", JSON.stringify(newGoal));
            return newGoal;
          });
        }
      }
  ...
  const pathLessons: PathLesson[] = useMemo(() => {
    ...
    console.log('🎯 Lesson Path Status:', {
      completedLessons: Array.from(completedLessons),
      firstUnlockedIndex,
      lessonStatuses: result.slice(0, 5).map(l => ({ id: l.id, title: l.title, status: l.status }))
    });
          ...
          <div>
            ...
            {showAbortConfirm && (
              <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
                ...
                >
                  <h2 className="text-2xl font-bold text-white mb-4">Leave Lesson?</h2>
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className="flex-1 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white px-6 py-3 rounded-xl font-semibold transition shadow-lg"
                    >
                      Leave Lesson
                    </button>
                  </div>
